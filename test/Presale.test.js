...
contract("Presale", async () => {
  ...
  it("Deploy contracts", async () => {
    ...
    presale = await Presale.deploy(token_a.address, token_b.address);
    await presale.deployed();
    console.log("Contract Address of Crowdale: ", presale.address);
  ...
  it("Check addresses of nonfungiblePositionManager in presale contract", async () => {
    var INonfungiblePositionManager =
      "0xC36442b4a4522E871399CD717aBDD847Ab11FE88";
    expect(await presale.nonfungiblePositionManager()).to.equal(
      INonfungiblePositionManager
    );
  });

  it("Set sale function in presale contract", async () => {
    var time = 12;
    var bonus = 8;
    var users = 6;
    await presale.set_sale(time, bonus, users);
    // expect(await presale.time_of_buy()).to.equal(time + block.timestamp());
    expect(await presale.early_bonus_token()).to.equal(bonus);
    expect(await presale.early_users()).to.equal(users);
  });

  it("reset_sale in presale contract", async () => {
    await presale.reset_sale();
    expect(await presale.time_of_buy()).to.equal(0);
    expect(await presale.early_bonus_token()).to.equal(0);
    expect(await presale.early_users()).to.equal(0);
  });

  it("resetInitialUsers in presale contract", async () => {
    await presale.resetInitialUsers();
    expect(await presale.initial_users()).to.equal(0);
  });

  it("getContractBalance in presale contract", async () => {
    expect(await presale.getContractBalance()).to.equal(1);
  });

  ////////////////// Token A //////////
  it("Presale contract end", async () => {});
  ...
  it("Check total supply and owner balance because initial supply is equal to owner balance in TOKEN_A contract", async () => {
    expect(await token_a.totalSupply()).to.equal(
  ...
  it("Check total supply and owner balance because initial supply is equal to owner balance in TOKEN_B contract", async () => {
    expect(await token_b.totalSupply()).to.equal(
      "120000000000000000000000000000"
    );
  });
